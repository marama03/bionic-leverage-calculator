              ...
              <div className="flex-1 flex justify-center md:justify-start w-full">
                <button
                  onClick={() => {
                    ...
                    const mp = Math.round(results.monthlyPenalty);
                    const vh = results.virtualHeadcount.toFixed(1);
                    const lag = metrics.responseTime;
                    ...
                    // 2. Get baseline input values
                    const R = metrics.revenue;
                    const H = metrics.headcount;
                    const C = metrics.costPerEmployee;
                    const W = metrics.coordinationHours;

                    // 3. Construct URL with all parameters
                    window.location.href = `${baseUrl}?tax=${tax}&leak=${leak}&eff=${eff}&mp=${mp}&vh=${vh}&lag=${lag}&R=${R}&H=${H}&C=${C}&W=${W}#top`;
                  }}
